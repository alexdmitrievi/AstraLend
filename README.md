...
`scripts/build-before-preview.sh` достаёт его из истории и переписывает
абсолютные пути под подпуть `/before/` — шаг workflow вызывает скрипт перед
загрузкой артефакта. Поэтому у `actions/checkout` стоит `fetch-depth: 0`:
без полной истории старый коммит в CI недоступен.

Чтобы убрать превью, достаточно удалить шаг «Превью версии до редизайна»
из `.github/workflows/pages.yml`.
